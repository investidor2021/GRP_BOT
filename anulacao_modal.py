...
def preencher_anulacao_empenho(page, row):
    ...
    texto_aba = "Liquidaç" if is_processado else "Documento de Despesa"
    nome_aba_log = "Liquidações" if is_processado else "Documento de Despesa"
    print(f"📂 Alternando aba da anulação para '{nome_aba_log}' (TipoResto='{tipo_resto}' -> Processado={is_processado})...")

    # Locators ESPECÍFICOS para a aba (EVITA selecionar div container pai da página)
    aba_selector = ".dx-tab, [role='tab'], span.dx-tab-text, div.tab-header, .dx-tab-content"
    
    try:
        aba = page.locator(aba_selector).filter(has_text=re.compile(texto_aba, re.IGNORECASE)).first
        if aba.is_visible(timeout=4000):
            print(f"🖱️ Clicando na aba '{nome_aba_log}'...")
            aba.scroll_into_view_if_needed()
            aba.click(force=True)
            page.wait_for_timeout(600)
        else:
            print(f"🔍 Aba não encontrada via selector específico. Tentando get_by_text em elementos inline...")
            page.locator("span, a, label, .dx-item-content").filter(has_text=re.compile(texto_aba, re.IGNORECASE)).first.click(force=True)
            page.wait_for_timeout(600)
    except Exception as ex_tab:
        print(f"⚠️ Aviso ao alternar aba '{nome_aba_log}': {ex_tab}")

    print(f"💰 Preenchendo o Valor na tabela da aba '{nome_aba_log}': {valor_val}")
    
    # DevExpress DataGrid: em grids DevExtreme, primeiro clica na célula da tabela para ativar a edição inline
    try:
        celula_td = page.locator(
            "dx-data-grid td[aria-colindex='6']:visible, "
            "dx-data-grid td.dx-cell-focus-disabled:visible, "
            "dx-data-grid td:visible"
        ).filter(has_text=re.compile(r'\d+')).first
        if celula_td.is_visible(timeout=3000):
            print("🖱️ Clicando na célula da tabela (para ativar edição)...")
            celula_td.click(force=True)
            page.wait_for_timeout(300)
    except Exception as ex_cell:
        print(f"ℹ️ Não foi necessário clicar na célula TD antes de focar input: {ex_cell}")

    # Captura o campo de input visível dentro da tabela
    campo_val = page.locator(
        "dx-data-grid input:visible, "
        "td[aria-colindex='6'] input:visible, "
        "td input.dx-texteditor-input:visible, "
        "td input[role='spinbutton']:visible"
    ).first
